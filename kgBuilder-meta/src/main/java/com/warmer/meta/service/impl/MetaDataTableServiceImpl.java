...
public class MetaDataTableServiceImpl implements MetaDataTableService {
  ...
  public List<DataTableVo> queryByDatasourceId(Integer datasourceId) {
    List<MetaDataTable> dataTable =
      this.metaDataTableDao.queryByDatasourceId(datasourceId);
    List<DataTableVo> voList = dataTable
      .stream()
      .map(n -> {
  ...
  public ArrayList<String> getMetaTables(MetaDataSource metaDataSource) {
    ...
    try {
      tables =
        DbUtils.getTables(
          dbType,
          metaDataSource.getDbCode(),
          userName,
          password,
          driverClassName,
          metaDataSource.getConnectUrl()
        );
    } catch (Exception e) {
      throw e;
    }
  ...
  public void saveTables(DataTableSubmitItem submitItem) {
    MetaDataSource dataSource = metaDataSourceDao.queryById(
      submitItem.getDataSourceId()
    );
    ...
    HashMap<String, ArrayList<HashMap<String, String>>> metaData = DbUtils.getMetaData(
      dbType,
      dataSource.getDbCode(),
      dataSource.getConnectUrl(),
      userName,
      password,
      driverClassName,
      submitItem.getDataTables()
    );
  ...
  public PageRecord<Map<String, Object>> getTableRecords(TableQuery queryItem) {
    MetaDataSource metaDataSource = metaDataSourceDao.queryById(
      queryItem.getDataSourceId()
    );
    String dbType = metaDataSource.getDbType();
    String userName = metaDataSource.getDbUserName();
    String password = metaDataSource.getDbPassWord();
    ...
    MetaDataTable metaDataTable = metaDataTableDao.queryByDatasourceIdAndTableCode(
      queryItem.getDataSourceId(),
      queryItem.getDataTableName()
    );
    List<DataColumnVo> dataColumnVos = metaDataColumnService.queryByTableId(
      metaDataTable.getDataTableId()
    );
    try {
      records =
        DbUtils.getTableInfoByPage(
          queryItem.getCurrentPage(),
          queryItem.getPageSize(),
          dbType,
          metaDataSource.getDbCode(),
          metaDataSource.getConnectUrl(),
          queryItem.getDataTableName(),
